...
export default function AddGoalModal({ visible, onClose, onAdd, goalToEdit }) {
    ...
    const handleAdd = () => {
        ...
        onAdd({ title: title.trim(), points: Math.round(points) });
        setTitle('');
        setPoints(50);
        onClose();
    };
        ...
        <Modal visible={visible} animationType="slide" transparent>
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.overlay}>
                <View style={styles.modal}>
                    ...
                    <Text style={styles.label}>Difficulty: {Math.round(points)} XP</Text>

                    <View style={styles.sliderContainer}>
                        <Text style={styles.sliderLabel}>Easy (50)</Text>
                        <Slider
                            ...
                            thumbTintColor="#fff"
                        />
                        <Text style={styles.sliderLabel}>Hard (300)</Text>
                    </View>
...
const styles = StyleSheet.create({
    ...
    sliderContainer: { flexDirection: 'row', alignItems: 'center', marginBottom: 30, gap: 10 },
    sliderLabel: { color: '#888', fontSize: 10, fontWeight: '700', textTransform: 'uppercase' },
